...
def _smib(*, unit_type: UnitType, controller: GfmController | None, bus_type: BusType) -> Network:
    """Shared builder for the three SMIB presets below."""
    buses = [
        Bus(id=1, name="gen_side", vn_kv=_CIGRE_MV_KV),
        Bus(id=2, name="ib_side", vn_kv=_CIGRE_MV_KV),
        Bus(id=3, name="der_terminal", vn_kv=_SMIB_LV_KV),
        Bus(id=4, name="ib_terminal", vn_kv=_SMIB_LV_KV),
    ]
    lines = [
    ...
    der_units = [
        DerUnit(
            id=1, bus=4, unit_type=UnitType.INFINITE_BUS, bus_type=BusType.SLACK,
    ...
    transformers = [
        Transformer(hv_bus=2, lv_bus=4, r_pu=_SMIB_TR_R_PU, x_pu=_SMIB_TR_X_PU, sn_mva=_CIGRE_SN_MVA, name="ib_xfmr"),
        Transformer(hv_bus=1, lv_bus=3, r_pu=_SMIB_TR_R_PU, x_pu=_SMIB_TR_X_PU, sn_mva=_CIGRE_SN_MVA, name="der_xfmr"),
    ]

    return Network(
        name=f"SMIB_{unit_type.value}",
        f_hz=50.0,
